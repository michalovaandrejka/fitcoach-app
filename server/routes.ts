...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/migrate/import-data", async (req, res) => {
    try {
      const { users, locations, bookings, availabilityBlocks, migrationKey } = req.body;
      
      if (migrationKey !== "fitcoach-migrate-2026") {
        return res.status(403).json({ error: "Invalid migration key" });
      }

      const results = { users: 0, locations: 0, bookings: 0, availabilityBlocks: 0 };

      for (const loc of locations || []) {
        try {
          const existing = await storage.getLocationById(loc.id);
          if (!existing) {
            await storage.createLocationWithId(loc.id, loc.name, loc.address, loc.isActive);
            results.locations++;
          }
        } catch (e) { console.log("Location exists:", loc.id); }
      }
      ...
      for (const user of users || []) {
        try {
          ...
          if (!existing) {
            await storage.createUserWithHash({
              id: user.id,
              email: user.email,
              name: user.name,
        ...
        } catch (e) { console.log("User exists:", user.email); }
      }

      for (const block of availabilityBlocks || []) {
        try {
          await storage.createAvailabilityBlockWithId(block.id, {
            date: block.date,
            startTime: block.startTime,
            endTime: block.endTime,
            branchId: block.branchId
          });
          results.availabilityBlocks++;
        } catch (e) { console.log("Block exists:", block.id); }
      }

      for (const booking of bookings || []) {
        try {
          await storage.createBookingWithId(booking.id, {
            date: booking.date,
            ...
            manualClientName: booking.manualClientName
          });
          results.bookings++;
        } catch (e) { console.log("Booking exists:", booking.id); }
      }
